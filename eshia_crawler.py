...
class EshiaSpider(scrapy.Spider):
    ...
    def ParseCats(self, response):
        # Use some tricks to ommit last two items => list[0:-2]
        cats_links = response.css("div#navigationBar > div:nth-child(1) > table > tr > td > ul > li > span > a::attr(href)").getall()[0:-2]
    ...
    def ParseSubCats(self, response):
        ...
        if sub_cats_links:
            yield scrapy.Request(url=sub_cats_links, callback=self.ParseData)
        # If subcategories does not exist get contents of page
        else:
            return scrapy.Request(url=response, callback=self.ParseData)
